...
fn main() {
    ...
    output_path.push("crates/windows_widgets/src/Microsoft");
    let _ = std::fs::remove_dir_all(&output_path);
    output_path.pop();

    let winmd_files = [File::new(".windows/winmd/Windows.Foundation.winmd").unwrap(), File::new(".windows/winmd/Windows.Win32.winmd").unwrap(), File::new(".windows/winmd/Windows.Win32.Interop.winmd").unwrap(), File::new(".windows/winmd/Microsoft.Windows.Widgets.winmd").unwrap()];
...
fn gen_tree(reader: &Reader, output: &std::path::Path, tree: &Tree) {
    ...
    tokens.push_str(r#"::core::include!("impl.rs");"#);
