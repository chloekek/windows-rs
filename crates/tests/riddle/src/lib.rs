...
pub fn winrt_riddle(input: &str) -> String {
    ...
    let output = std::env::temp_dir().join("riddle").join(std::path::Path::new(input).with_extension("winmd").file_name().expect("file_name failed")).to_string_lossy().into_owned();
